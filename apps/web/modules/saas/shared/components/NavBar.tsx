...
export function NavBar() {
	...
	return (
		<nav
			className={cn("w-full border-b-2", {
		...
		>
			<div
				className={cn("container py-4", {
					"py-4 md:flex md:h-full md:flex-col md:px-6 md:pt-6 md:pb-0":
						useSidebarLayout,
				})}
			>
				...
				<ul
					className={cn(
						"no-scrollbar -mb-4.5 mt-4 flex list-none items-center justify-start gap-4 overflow-x-auto text-sm",
						{
					...
					{menuItems.map((menuItem) => (
						<li key={menuItem.href}>
							<Link
								href={menuItem.href}
								className={cn(
									"flex items-center gap-2 whitespace-nowrap border-b-2 px-1 pb-2",
									[
										menuItem.isActive
